        ...
        for line in linksFile:
            if "#Link" in line:
                ...
                if "DOCADS.NSF" in link_intern:  # Mirem si es link a mateixa BBDD
                    ...
                    replacedContent = replacedContent.replace(("Database 'ADS - SPO', View 'By Category', Document '"), '')
                    ...
                    objecte.indexObject()
                    transaction.commit()
                    changesFile.write('LinkChanged_in: ' + objecte.absolute_url() + '\n')
                    logging.info("LinkChanged_in: %s ", objecte.absolute_url())
